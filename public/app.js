...
class EnigmaUI {
    ...
    attachEventListeners() {
        ...
        this.loadSettingsBtn.addEventListener('click', () => this.loadDefaultSettings());
        ...
        this.plugboardInput.addEventListener('input', () => this.validatePlugboard());
    ...
    getSettings() {
        const plugboardPairs = this.plugboardInput.value.trim()
            .split(/\s+/)
            .filter(p => p.length === 2);
    ...
    async processMessage() {
        ...
        try {
            ...
            alert('Error communicating with Enigma server. Make sure the server is running on port 4000.');
        }
    }
    ...
    advanceRotors() {
        ...
        if (this.currentRotorPositions[2] === 0) {
            ...
            if (this.currentRotorPositions[1] === 0) {
                this.currentRotorPositions[0] = (this.currentRotorPositions[0] + 1) % 26;
            }
        }
    ...
    updateRotorDisplays() {
        this.leftRotorPosition.textContent = this.positionToLetter(this.currentRotorPositions[0]);
        this.middleRotorPosition.textContent = this.positionToLetter(this.currentRotorPositions[1]);
        this.rightRotorPosition.textContent = this.positionToLetter(this.currentRotorPositions[2]);
    }
    ...
    async copyOutput() {
        try {
            ...
            this.copyBtn.style.background = 'linear-gradient(145deg, #4caf50, #388e3c)';
